...
sub checkInput {
	...
	die "Blast :: The E-Value Threshold given ($ethreshold) is not valid.\n".
	    "      :: Must be a power of 10 between 0.0001 and 10000.\n".
	    "      :: Use \"Blast.pl -h\" for help.\n\n";
    }
...
#    open HTML, ">${fnpfx}.html"; print HTML "$html"; close HTML;
    ...
    open URL,  ">${fnpfx}.url";  print URL  "$url\n"; close URL;
...
################################################################################
sub flagCutNonGPCR {
    # List of Accession Numbers matching UniProt Keyword KW-0297 (GPCR)
    my $url = "http://www.uniprot.org/uniprot/?query=keyword\%3aKW-0297\&format=list";
    my $db  = get "$url";
    die "Blast :: Could not load GPCR Listing :: $url\n" unless defined $db;
    my @gpcraccs = split(/\n/, $db);
    my %is_gpcr  = ();
    foreach (@gpcraccs) { $is_gpcr{$_} = 1; }
    ...
    # Parse results into GPCR/non-GPCR
    while (@results) {
	my $result = shift @results;
	my $evalue = shift @evalues;

	if (! defined $is_gpcr{$result}) {
	    if ($flagnongpcr) {
	...
	if ($flagnongpcr) {
	    $printstring .=
		"Potential Non-GPCRs ::\n".
		" :: The following $count sequences may not be GPCRs.\n".
		" :: They were not found in the UniProt GPCR (KW-0297) list:\n".
		" ::   http://www.uniprot.org/uniprot/?query=keyword\%3aKW-0297\&format=list\n".
		" ::";
	} elsif ($cutnongpcr) {
	    $printstring .=
		"Potential Non-GPCRs ::\n".
		" :: The following $count sequences may not be GPCRs and were cut from the results.\n".
		" :: They were not found in the UniProt GPCR (KW-0297) list:\n".
		" ::   http://www.uniprot.org/uniprot/?query=keyword\%3aKW-0297\&format=list\n".
		" ::";
	}
	
	# Print non-GPCR FASTAs
	my $nongpcrfasta = "";
	for (my $i = 0; $i < @nongpcr_results; $i++) {
	    my $accurl = "http://www.uniprot.org/uniprot/$nongpcr_results[$i].fasta";
	    my $tmp = get $accurl;
	    if (! defined $tmp) {
	...
	open NON, ">${fnpfx}.non_gpcr.fta";
	print NON $nongpcrfasta;
	close NON;
...
sub loadFASTAs {
	...
	my $accurl = "http://www.uniprot.org/uniprot/$results[$i].fasta";
	my $tmp = get $accurl;
	if (! defined $tmp) {
	    die "Blast.pl :: Could not load FASTA results.\n".
		"         :: $accurl\n";
...
#	print "$i\n";
	...
	$printstring = 
	    ...
	    " :: BLAST HTML Output             :: ${fnpfx}.html\n".
	    " :: BLAST XML Output              :: ${fnpfx}.xml\n".
	    " :: Flagged/Cut Non-GPCRs FASTA   :: ${fnpfx}.non_gpcr.fta\n".
	    " :: Blast.pl Log File             :: ${fnpfx}.log\n";
    } else {
	$printstring = 
	    "Output Files ::\n".
...
Optional Input:
 ...
 :: webpage at:
 ::   http://www.uniprot.org/uniprot/?query=keyword\%3aKW-0297\&format=list
 :: It is possible that some GPCR sequences will be flagged

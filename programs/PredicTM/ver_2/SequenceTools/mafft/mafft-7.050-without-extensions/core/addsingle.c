...
//			fixed_musclesupg_float_realloc_nobk_halfmtx( njobc, iscorec, topolc, lenc, depc, 0 );
			neighbor = addonetip( njobc, topolc, lenc, iscorec, topol, len, dep, treeout, addtree, iadd, name );
		...
		for( j=i+1; j<njob; j++ ) 
		{
			...
			if( j < norg )
			{
				if( bunbo == 0.0 )
					...
					imtx[i][j-i] = ( 1.0 - mtxv / bunbo ) * lenfac;
			...
			else
			{
				if( bunbo == 0.0 )
					...
					nmtx[i][j-norg] = ( 1.0 - mtxv / bunbo ) * lenfac;
			}
		} 
		free( table1 );

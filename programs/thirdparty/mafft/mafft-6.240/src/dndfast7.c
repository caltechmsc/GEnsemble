...
//		fprintf( stderr, "###  i = %d\n", i );
		...
		if( scoremtx == -1 )
			sprintf( com, "%s %s -z3 -m10  -n -Q  -b%d -E%d -d%d %s %s %d > %s\0", fastapath, swopt, M, M, M, queryfile, datafile, ktuple, fastafile );
		else
			sprintf( com, "%s %s -z3 -m10  -Q  -b%d -E%d -d%d %s %s %d > %s\0", fastapath, swopt, M, M, M, queryfile, datafile, ktuple, fastafile );
		res = system( com );

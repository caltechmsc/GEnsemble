...
def scfclosed(atoms,F0,nclosed):
...
def scfopen(atoms,F0,nalpha,nbeta):
...
def scf(atoms,chg=0,mult=None):
    ...
    F0 = get_F0(atoms)
    if nopen:
        Eel = scfopen(atoms,F0,nclosed+nopen,nclosed)
    else:
        Eel = scfclosed(atoms,F0,nclosed)
    Etot = Eel+Enuke
    Hf = Etot*ev2kcal+eref
    if VERBOSE: print "Final Heat of Formation = ",Hf
    return Hf

def get_energy_forces(atoms,chg=0,averaging=1):

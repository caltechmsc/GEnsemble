...
def validate(xmlf, reportf, phase=None):
    ...
    if KEYS or RESULT_TREE_PROCESSOR:
...
def command_line(args):
    ...
    parser.add_option("-p", "--phase",
                      ...
                      help="Execute the schem in the specified phase", metavar="XML_ID")
    global OPTIONS, ARGS
...
def run(stron_inf, validator_outf, legacy_ns=False, prep_for_test=0):
    ...
    source = InputSource.DefaultFactory.fromStream(stron_inf, dummy_uri)
    parser.parse(source)
...
def main(argv=[__name__]):
    ...
    try:
        ...
        run(stronf, outwrap(fout), OPTIONS.legacy_ns, OPTIONS.test_ready)
    except Usage, err:
        print >>sys.stderr, err.msg
        return 2

...
#from cmdf.imd.imdForceEngine import IMDForceEngine
import cmdf.mee.mee as mee
import cmdf.reaxFF.reax as reax
#import cmdf.imd.IMD_EAM_noshift as imd_eam 
...
#from imdTools import checkOpen, Rec, Exp, Sqrt
...
class Control:
    ...
    #def setForceEngine(self, valenceForceEngine, coulombForceEngine, vdwForceEngine, solvationForceEngine):
        ...
        if ForceEngine is MeeForceEngine:
        ...
        elif ForceEngine is IMDForceEngine:
            FFflavor = self.imdEamFSP['FFflavor']
            cutoff = self.imdEamFSP['cutoff']
            spacing = self.imdEamFSP['spacing']
            imdEamFFName = util.searchFileName(os.path.join('cmdf', 'imd', 'imdEamForceField'))
            self.forceEngine = IMDForceEngine(self.numAtoms, self.cellDim)
            ...
            energy, forces = self.forceEngine.calcEnergyForce(self.coords)
            print 'Initial IMD-EAM energy (kcal/mol)  =', energy / unit.kcalpermol
            print 'Initial IMD-EAM force (kcal/mol/A) =', forces / unit.kcalpermol
            imd_eam.PrintEnergies()
            logFile = open(self.molName+'.log', 'a') 
            logFile.write('=> Setup IMD force engine completed.\n')
            logFile.close()
            ...
            if line.find('FORCE_ENGINE') == 0:
                ...
                if len(fields) >= 2:
                    if fields[1] == 'mee':
                        ...
                        self.forceEngine = IMDForceEngine
                        system.setPeriodic(1) 
    ...
    def writeKey(self):
        ...
        if self.printLevel > 0:
            ...
            elif self.forceEngine is IMDForceEngine:
                logFile.write('Force engine            : IMD-EAM \n')

...
#                                                                                       #
#  Created on 02/01/06, last revision on 09/05/06. yi                                   #
#                                                                                       #
#########################################################################################
...
from cmdf.dynamics.distribution import Random1, Random2, Random3, Gaussian1, Gaussian2, Gaussian3
...
class Thermostat:
    ...
    def run(self, energy, degFree, mass, v):
...
#       
    def __init__(self, targetTemp=300., timeStep=0.1, tauTemp = 10.):
...
#
    def __init__(self, targetTemp=300., timeStep=0.1, niuTemp=0.01, gaussian=Gaussian3, imove=numpy.zeros(1, numpy.Int)):
        Thermostat.__init__(self, targetTemp)
        self.timeStep = timeStep
    ...
    def run(self, energy, degFree, mass, v):
        ...
        if ranum < (self.niuTemp * self.timeStep):  # Rate of collison with bath 
            #from utility_pf import Utility
            util = Utility()
            util.setMBvelocity(self.imove, mass, v, self.targetTemp, self.gaussian) 
             
        return v

class NoseHooverThermostat(Thermostat):

    def __init__(self, molName='CMDF', targetTemp=300., timeStep=0.1, numNose=4, freqNose=0.1, numSuz=3):
    ...
    #
        ...
        kenh = 0.5 * kenh  
        # Note that energy is for entir system rahter than thermo regions
        print 'Warning! Nose-Hoover: Note that energy is for entir system rahter than thermo regions!'
        pepart = energy 
        penh0 = gnkt * xnh[0] 
        penhi = 0.

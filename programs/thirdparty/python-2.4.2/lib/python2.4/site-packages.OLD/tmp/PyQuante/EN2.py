...
from Numeric import zeros,Float,dot,sqrt, choose
from PyQuante.LA2 import mkdens, GHeigenvectors
from PyQuante.basis_ccpvtz import basis
...
def TransformInts(Ints,orbs1,orbs2, nocc):
    ...
    temp = zeros((nbf,nbf,nbf,nbf),Float)
    tempvec = zeros(nbf,Float)
    for mu in bfs:
        for nu in bfs:
            for eta in bfs:
                for b in bfs:
    ...
    temp2 = zeros((nbf,nbf,nbf,nbf),Float)
    for nu in bfs:
        for eta in bfs:
            for b in bfs:
                for a in bfs:
...
def EN2(atomlist,charge=0,unpaired = 0, ConvCriteria=0.00005,MaxIter=20):
    "General wrapper for the simple CI method"

    #the initial unrestricted Hatree-Fock computation
    ...
    Yalpha = zeros((nbf),Float)
    Ybeta = zeros((nbf),Float)

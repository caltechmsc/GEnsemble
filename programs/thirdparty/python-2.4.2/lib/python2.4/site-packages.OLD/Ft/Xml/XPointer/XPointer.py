...
_xpointerSchemeParser = XPtrExprParserc.new()
del XPtrExprParserc
...
class Shorthand(Pointer):
    ...
    def select(self, context):
        ...
        if hasattr(doc, 'getElementById'):
            ...
            element = Util.GetElementById(doc, self.identifier)
        if element is None:
            return []
        return [element]
...
class ElementScheme(PointerPart):
    ...
    def evaluate(self, context):
        ...
        if self.identifier:
            if hasattr(node, 'getElementById'):
                node = node.getElementById(self.identifier)
            else:
                # May raise ValueError
                node = Util.GetElementById(node, self.identifier)
            if node is None:
                return []

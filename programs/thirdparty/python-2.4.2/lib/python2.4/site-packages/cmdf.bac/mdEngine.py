...
#                                                                                   #
# Created on 01/22/06, last revision on 06/05/06. yi                                #
#                                                                                   #
#####################################################################################
import datetime
import numpy 
...
class MDEngine:
    ...
    def __init__(self, integrator=None, thermostat=None, barostat=None,  \
        gaussian=Gaussian3, numSteps=1000, freqOutputMD=10, freqOutputTrj=10, initTemp=600.):  
    ...
    # Initialize position and velocity of molecule.
        ...
        cellDim, elements, coords, vels, energy, forces, oldForces, imove, fixed, charges,  \
            masses, fftypes, bondlist, orderlist = systemToArrays(system)

        # Translate the center of mass to origin.
        origin = numpy.zeros(3, numpy.Float)
        coords = utility_f.translatecm(masses, coords, origin)

        # Assign velocity from Maxwell-Boltzmann distribution.
        util.setMBvelocity(imove, masses, vels, self.initTemp, self.gaussian)
    ...
    def run(self, system, forceEngine):
        ...
        for i in range(1, mdSteps+1):  # MD main loop
            ...
            if isinstance(self.thermostat, NoseHooverThermostat):
                self.thermostat.run(molecule)

            oldForces = forces.copy()
            coords, vels, energy, forces = self.integrator.run(forceEngine, coords, vels, forces)  # MD integration. 

            if self.thermostat:  # Thermostat
                vels = self.thermostat.run(energy, degFree, vels)
            ...
            if i % freqOutputTrj == 0:  # Output trajectory and restart files.
                util.outputTrj(xyzFile, elements, coords, vels, forces, i)
                self.writeMDrestart(rstName, molName, cellDim, coords, vels, forces, oldForces)

            if i % freqOutputTrj == 0 and  \
                not isinstance(self.thermostat, AndersenThermostat):  
                util.removeTranslation(molName, imove, masses, vels)
                ### if not PBC    
                util.removeRotation(molName, imove, masses, coords, vels)  

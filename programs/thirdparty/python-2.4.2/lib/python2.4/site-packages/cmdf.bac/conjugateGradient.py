...
class ConjugateGradient_SP(ConjugateGradient):
    ...
    def __init__(self, molName='cmdf', nMethod=0, numSteps=1000, threshold=0.01, freqOutput=100, degFree=3, coords=zeros([1],Float)):
        ConjugateGradient.__init__(self, molName, nMethod, numSteps, threshold, freqOutput, degFree, coords)
        return
    ...
    */
    ...
    def run(self, system, forceEngine):
        ...
        #g = [0.] * degFree
        #w = [0.] * mdim
        x = zeros(degFree, Float)
        g = zeros(degFree, Float)
        w = zeros(mdim, Float)
        acc = 1.e-20
        #eps = .00001
        iout = 1
        ...
        miniFile = open(molName+'.min', 'a')
        if nmeth == 0:
            miniFile.write('\t\t<<< CMDF minimization via conjugate gradient method >>>\n')
        elif nmeth == 1:
            miniFile.write('\t\t<<< CMDF minimization via BFGS quasi-newton method >>>\n')

...
class MpeForceEngine:
    def __init__(self, natoms, cellDim, sgname, sgsymbol, sgnumber, axes):
        self.natoms = natoms
        self.energy = 0.0
    ...
    def setupEnergyExpression(self, coords, fixed, charges, fftypes, blist, bolist, itrlist):
        a,b,c,alpha,beta,gamma = self.cellDim[0:6]
        ...
        for i in range(self.natoms):
            ...
            itranlist = itrlist[start:end]

            MEE.cmdf_mee_set_atom2(i+1, x, y, z, fixed[i], charges[i], fftypes[i], bondlist, orderlist, itranlist)

        
        print "loading done"
        MEE.cmdf_mpe_init(self.natoms, a, b, c, alpha, beta, gamma, 
                          self.sgname, self.sgsymbol, self.sgnumber, 
                          self.axes)

        MEE.cmdf_mpe_setup_energy_expression()
        return 
    ...
    def calcEnergyForce(self):
        ...
        self.energy = MEE.cmdf_mpe_get_energy() * unit.kcalpermol
        return
    ...
    def getEnergy(self):
        return self.energy
        #unit = Unit()
        #return MEE.cmdf_get_energy() * unit.kcalpermol


    def getForces(self):
        forces = numpy.zeros(3*self.natoms, numpy.Float)
        for i in range(self.natoms):
            forces[3*i] = -MEE.cmdf_get_force_x(i+1)
            forces[3*i+1] = -MEE.cmdf_get_force_y(i+1)
            forces[3*i+2] = -MEE.cmdf_get_force_z(i+1)
        return forces * unit.kcalpermol

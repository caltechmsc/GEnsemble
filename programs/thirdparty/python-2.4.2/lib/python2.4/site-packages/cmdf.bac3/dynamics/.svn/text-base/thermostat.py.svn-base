...
#                                                                                       #
#  Created on 02/01/06, last revision on 03/27/06. yi                                   #
#                                                                                       #
#########################################################################################
...
#       
    ...
    def run(self, molecule):
        ...
        UTI = Utility()
        currentTemp = UTI.calcTemp(molecule)
        if currentTemp == 0.:
            currentTemp = 1.e-5
...
#
    def __init__(self, targetTemp=300., timeStep=0.1, niuTemp=0.01, gaussian=Gaussian3):
        Thermostat.__init__(self, targetTemp)
        self.timeStep = timeStep
    ...
    def run(self, molecule):
        ...
        if ranum < (self.niuTemp * self.timeStep):  # Rate of collison with bath 
            from utility import Utility
            UTI = Utility()
            UTI.setMBvelocity(molecule, self.targetTemp, self.gaussian) 
             
        return

class NoseHooverThermostat(Thermostat):

    def __init__(self, targetTemp=300., timeStep=0.1, numNose=4, freqNose=0.1, numSuz=3):
        Thermostat.__init__(self, targetTemp)
        self.timeStep = timeStep
    ...
    #
        ...
        if (count % 2 == 0.):
            num = count / 2
            nhFile = open(molecule.getName()+'.nh', 'a')

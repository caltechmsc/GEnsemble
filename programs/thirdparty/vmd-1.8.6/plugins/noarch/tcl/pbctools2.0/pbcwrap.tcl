...
#
# $Id: pbcwrap.tcl,v 1.5 2007/03/02 15:58:32 johns Exp $
#
...
namespace eval ::PBCTools:: {
	...
	for { set frame $first } { $frame <= $last } { incr frame } {
	    ...
	    set cell [lindex [ pbcget -namd -now -molid $molid ] 0]
	    set A   [lindex $cell 0]
	    set B   [lindex $cell 1]
	    set C   [lindex $cell 2]
	    set Ax [lindex $A 0]
    ...
    ########################################################
    proc wrap_to_rectangular_unitcell { molid Ax By Cz origin wrapsel } {

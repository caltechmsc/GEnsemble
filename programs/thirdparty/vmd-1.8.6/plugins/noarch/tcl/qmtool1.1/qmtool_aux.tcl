...
#
# $Id: qmtool_aux.tcl,v 1.18 2006/10/18 00:31:14 saam Exp $
#
...
proc ::QMtool::find_improper_dihedrals { zmat } {
   set ndihed  [lindex [lindex $zmat 0] 4]
   set nimprop [lindex [lindex $zmat 0] 5]
   set index 0
   foreach entry $zmat {
...
proc ::QMtool::vmd_clear_bondlist {molid} {
...
proc ::QMtool::vmd_addbond {atom0 atom1 molid} {
   set sel [atomselect $molid all]
...
proc ::QMtool::vmd_delbond {atom0 atom1 molid} {
   set sel [atomselect $molid all]
   set bondlist [$sel getbonds]
...
proc ::QMtool::modredundant_zmat { {seltext all} } {
   ...
   set bondlist  [bondlist $molid $seltext]
   set anglelist [anglelist [$sel list] $neighborlist]

   if {$autogendiheds=="one"} {
      set dihedlist [dihedlist [$sel list] $bondlist $neighborlist]
   } else {
      set dihedlist [completedihedlist [$sel list] $bondlist $neighborlist]
   }
   ...
   foreach ilist $bondlist {
	 ...
	 set found [lsearch -regexp $zmat "bond\\s+\\{[string map {{ } {\s}} $ilist]\\}"]
      ...
      incr nbonds
      set name "R$nbonds"
      set val [measure_bond $molid $ilist]
   ...
   foreach ilist $anglelist {
	 ...
	 set found [lsearch -regexp $zmat "(angle|lbend)\\s+\\{[string map {{ } {\s}} $ilist]\\}"]
      ...
      incr nangles
      set name "A$nangles"
      set val [measure_angle $molid $ilist]
   ...
   foreach ilist $dihedlist {
	 ...
	 set found [lsearch -regexp $zmat "dihed\\s+\\{$ilist\\}"]
      ...
      incr ndiheds
      set name "D$ndiheds"
      set val [measure_dihed $molid $ilist]
...
proc ::QMtool::bondlist {{molid top} {seltext all}} {
   set sel [atomselect $molid $seltext]
...
#############################################################
# Returns a list of triples defining bond angles.           #
#############################################################

proc ::QMtool::anglelist { atomlist neighborlist } {
   set anglelist {}
   foreach neighborperatom $neighborlist center $atomlist {
...
###############################################
# Generate one dihedral angle per bond.       #
###############################################
proc ::QMtool::dihedlist { atomlist bondlist neighborlist } {
   set dihedlist {}
   foreach bond $bondlist {
...
##############################################
# Generate a complete dihedral list.         #
##############################################
proc ::QMtool::completedihedlist { atomlist bondlist neighborlist } {
   set dihedlist {}
   foreach bond $bondlist {
      set atom1 [lindex $bond 0]
      set atom2 [lindex $bond 1]
...
proc ::QMtool::atomnum2element { symbol } {
   set periodic {{} H He Li Be B C N O F Ne Na Mg Al Si P S Cl Ar K Ca Sc Ti V Cr Mn Fe Co \
		   Ni Cu Zn Ga Ge As Se Br Kr Rb Sr Y Zr Nb Mo Tc Ru Rh Pd Ag Cd In Sn Sb \
		   Te I Xe Cs Ba La Ce Pr Nd Pm Sm Eu Gd Tb Dy Ho Er Tm Yb Lu Hf Ta W Re Os \
		   Ir Pt Au Hg Tl Pb Bi Po At Rn}

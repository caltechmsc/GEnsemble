...
namespace eval ::Blast {
    ...
    proc searchDatabase {sequenceIDs database {cutoff 10.0} {iterations 1} {maxResults 500}} {
        ...
        if {[llength $sequenceIDs] == 1} {
            ...
            # Run blast.
            set output [run $tempDir blastpgp -d $database -e $cutoff -i $inputFilename -o $outputFilename -m 6 -b $maxResults -j $iterations]
        ...
        # This must be a profile search.
            ...
            # Run blast.
            set output [run $tempDir blastpgp -d $database -e $cutoff -i $targetFilename -B $profileFilename -o $outputFilename -m 6 -b $maxResults -j $iterations]

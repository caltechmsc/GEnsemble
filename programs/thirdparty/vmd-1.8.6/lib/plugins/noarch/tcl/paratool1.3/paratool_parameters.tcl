   ...
   foreach t $zmat {
	 ...
	 2 { 
	    ...
	    if {[llength [join $bondedpar]]} {
	       ...
	       lset ::Paratool::zmat $i 5 "[regsub {[QCRM]} [lindex $::Paratool::zmat $i 5] {}]C"
	    } else {
...
proc ::Paratool::choose_analog_parameters_gui {} {
   ...
   label $w.types.format -font {tkFixed 9} -textvariable ::Paratool::chooseanalogformat \
      -relief flat -bd 2 -justify left;

   frame $w.types.list
   scrollbar $w.types.list.scroll -command "$w.types.list.list yview"
   listbox $w.types.list.list -yscroll "$w.types.list.scroll set" -font {tkFixed 9} \
   ...
   button $w.buttons.add -text "Use parameters"    -command {
	 ...
	 lset ::Paratool::zmat $izmat 4 [lrange $::Paratool::chooseanaloglist $i end-3 end-1]; # Kchi, n, delta

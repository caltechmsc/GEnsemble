...
proc ::Paratool::Energy::parabolic_extrapolation {a b c fa fb fc} {
   ...
   if {abs($q)<1e-16} { 
      error "parabolic_extrapolation: The 3 test points are collinear! (denom=$q)"
   }
   return [expr {$b - $p/$q}]
}
...
proc ::Paratool::Energy::make_distortion {molid type arrpos arratom args} {
   ...
   if {[string match "*bond" $type]} {
	 ...
	 set base(-2) [lindex [::Paratool::ring_order [lindex $ringlist $inring] $base(0) $base(-1)] 2]
	 ...
	 set indexes1 [::Paratool::ldiff [::Paratool::bondedsel $molid $atom(0) $vis] $vis]
	 ...
	 $sel delete
	 set atom(2) [::Paratool::ldiff $vis $atom(0)]
	 #puts "base(2)=$base(2); atom(2)=$atom(2)"
	 set base(3) [lindex [::Paratool::ring_order [lindex $ringlist $inring] $base(1) $base(2)] 2]
	 set atom(3) $atomtrans($base(3))
	 #puts "base(3)=$base(3); atom(3)=$atom(3)"
	 set pos(2)  $postrans($base(2))
	 set pos(3)  $postrans($base(3))
	 set indexes2 [::Paratool::ldiff [::Paratool::bondedsel $molid $atom(1) $vis] $vis]
	 set type bondring
	 #puts "$atom(1): vis=$vis, [::Paratool::bondedsel $molid $atom(1) $vis] indexes2=$indexes2"
      } else {
	 set indexes1 [::Paratool::bondedsel $molid $atom(0) $atom(1)]
	 set indexes2 [::Paratool::bondedsel $molid $atom(1) $atom(0)]
      }
	 ...
	 set indexes1 [::Paratool::bondedsel $molid $atom(0) $atom(1)]
	 set indexes2 [::Paratool::bondedsel $molid $atom(2) $atom(1)]
	 set sel1 [atomselect $molid "index $indexes1 and not index $atom(1)"]
	 set sel2 [atomselect $molid "index $indexes2 and not index $atom(1)"]      
      ...
      set indexes1 [::Paratool::bondedsel $molid $atom(1) $atom(2)]
      set indexes2 [::Paratool::bondedsel $molid $atom(2) $atom(1)]
   ...
   if {[string match "*bond" $type]} {
      ...
      set xupper [::QMtool::measure_bond $molid [list $atom(0) $atom(1)] [expr {$last+2}]]
      set x  [veclength $bondvec]
      set del [expr {($xupper-$x)/2.0}]
   } elseif {[regexp "anglering" $type]} {
      #set x  [QMtool::measure_angle $molid [list $atom(0) $atom(1) $atom(2)]]
      ...
      #set x [QMtool::measure_dihed $molid [list $atom(0) $atom(1) $atom(2) $atom(3)]]
      ...
      #set x [QMtool::measure_imprp $molid [list $atom(0) $atom(1) $atom(2) $atom(3)]]
      set x [::Paratool::dihed_from_coords $pos(0) $pos(1) $pos(2) $pos(3)]
   } else { return }
...
proc ::Paratool::Energy::start_namdserver {sel1 {etype "-all"} {rebuild "-rebuild"}} {
   ...
   # Write new NAMD input file
   if {![llength $::Paratool::newparamfile]} {
      #tk_messageBox -icon error -type ok -title Message -parent .charmmcharge \
 	 -message "You must first create a parameter file!"
      #return 0
   }
...
proc ::Paratool::Energy::update_14_list {} {
   ...
   foreach i [$sel get index] onetwo [$sel getbonds] {
      set onefour  [lindex [::Paratool::n-bondedsel $molidbase $i {} 4] 1]
      set onethree [lindex [::Paratool::n-bondedsel $molidbase $i {} 3] 1]

      lappend onefourlist [::Paratool::ldiff [::Paratool::ldiff $onefour $onethree] $onetwo]
   }
...
proc ::Paratool::Energy::compute_13_exclusions {} {
   ...
   foreach i [$sel get index] {
      lappend excluded13 [::Paratool::bondedsel $molidbase $i {} 3]
   }
...
proc ::Paratool::Energy::update_selected_geometries {molid} {
   variable selgeomlist
   [QMtool::measure_bond $molid [list $atom(0) $atom(1)]]
}

...
if (($nonminonly == 0) && (not defined $nonminonly)){
...
else {`/project/Biogroup/Software/GEnsemble/programs/superbihelix/run_super_create_nonmin.pl -b $bgf -m $mfta -p $tot -i $output_final --range $range --ppn $ppn`}
                      }
else {
if (($bio != 0) || (defined $bio)) { 
                                   system("cp Super_Neutralize_rank$sortrank.out $masterdir");

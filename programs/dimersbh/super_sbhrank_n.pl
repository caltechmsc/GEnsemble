...
#$out = "Super_CombiHelix_sbhrank" . $proc . ".out";
#open(OUT,">$out");

### NEUTRALIZE ###
#$outneut = "Super_Neutralize_sbhrank" . $proc . ".out";
...
if ($fin == 1)
{
  for ($k = 1; $k <= $proc; $k++)
  {
    $file = "super_sbhrank_" . $split_totin2[0] . "_" . "$k/combi_done";
    while ($dn < $k)
    { 
      if (-e $file)
      {
  ...
  sleep(10);
  $master_dir = "super_sbhrank_" . $split_totin2[0];
#  chdir("..");
  system("mkdir $master_dir");
  for ($z = 1; $z <= $proc; $z++)
  {
    $bgfs_dir = "super_sbhrank_" . $split_totin2[0] . "_" . $z;

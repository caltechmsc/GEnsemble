...
if (!$prefix | !$nostruc)
{die "\nProgram to analyze output from bihelixrot.
18 October 2008 Jenelle Bray (jenelle@wag.caltech.edu)
...
This script will calculate the total energy (TotalE), the energy excluding
interhelical van der Waals (NoInterHelVDW), interhelical energy (InterHelE), and
interhelical hydrogen bond energy (HBond) of all N^7 structures whose pairwise
 energies were calculated with the bihelixrot_scream script. 
The prefix is the prefix from the protein 
...
Output: prefix.bihelix_TotalE.out
	...
	prefix.bihelix_InterHelE.out
	prefix.bihelix_HBond.out
...
chomp $f1_2_dir;
chdir($f1_2_dir);
$f1_2 = $prefix . "_H1-H2_bihelixrot_$geten.out";
if (! -e $f1_2)
...
chomp $f1_7_dir;
chdir($f1_7_dir);
$f1_7 = $prefix . "_H1-H7_bihelixrot_$geten.out";
if (! -e $f1_7)
...
$noline2 = @f17;
if ($noline2 != $noline)
{die "Each bihelix output file has different number of 2-helix structures \n"}
close (F17);
chdir("..");
...
chomp $f2_3_dir;
chdir($f2_3_dir);
$f2_3 = $prefix . "_H2-H3_bihelixrot_$geten.out";
if (! -e $f2_3)
...
chomp $f2_4_dir;
chdir($f2_4_dir);
$f2_4 = $prefix . "_H2-H4_bihelixrot_$geten.out";
if (! -e $f2_4)
...
chomp $f2_7_dir;
chdir($f2_7_dir);
$f2_7 = $prefix . "_H2-H7_bihelixrot_$geten.out";
if (! -e $f2_7)
...
chomp $f3_4_dir; 
chdir($f3_4_dir);
$f3_4 = $prefix . "_H3-H4_bihelixrot_$geten.out";
if (! -e $f3_4)
...
chomp $f3_5_dir;
chdir($f3_5_dir);
$f3_5 = $prefix . "_H3-H5_bihelixrot_$geten.out";
if (! -e $f3_5)
...
chomp $f3_6_dir;
chdir($f3_6_dir);
$f3_6 = $prefix . "_H3-H6_bihelixrot_$geten.out";
if (! -e $f3_6)
...
chomp $f3_7_dir;
chdir($f3_7_dir);
$f3_7 = $prefix . "_H3-H7_bihelixrot_$geten.out";
if (! -e $f3_7)
...
chomp $f4_5_dir;
chdir($f4_5_dir);
$f4_5 = $prefix . "_H4-H5_bihelixrot_$geten.out";
if (! -e $f4_5)
...
chomp $f5_6_dir;
chdir($f5_6_dir);
$f5_6 = $prefix . "_H5-H6_bihelixrot_$geten.out";
if (! -e $f5_6)
...
chomp $f6_7_dir;
chdir($f6_7_dir);
$f6_7 = $prefix . "_H6-H7_bihelixrot_$geten.out";
if (! -e $f6_7)
...
for ($tm7 = 0; $tm7 < $nost; $tm7++)
{
	...
	$tot_al = $$fc32{$tm2a} + $$fc34{$tm4a} + $$fc35{$tm5a} + $$fc36{$tm6a} + $$fc37{$tm7a} + $$fc71{$tm1a} + $$fc72{$tm2a} + $$fc76{$tm6a} + $$fc12{$tm2a} + $$fc65{$tm5a} + $$fc24{$tm4a} + $$fc54{$tm4a};
	$tot_al1 = $tot_al + $tot_1 + $tot_2 + $tot_3 + $tot_4 + $tot_5 + $tot_6 + $tot_7;
	$tot_all = sprintf("%.3f",$tot_al1) + $fudge; 

	$tot_somp = $$fv32{$tm2a} + $$fv34{$tm4a} + $$fv35{$tm5a} + $$fv36{$tm6a} + $$fv37{$tm7a} + $$fv71{$tm1a} + $$fv72{$tm2a} + $$fv76{$tm6a} + $$fv12{$tm2a} + $$fv65{$tm5a} + $$fv24{$tm4a} + $$fv54{$tm4a};
	$tot_some1 = $tot_somp + $tot_1 + $tot_2 + $tot_3 + $tot_4 + $tot_5 + $tot_6 + $tot_7;
        $tot_some = sprintf("%.3f",$tot_some1) + $fudge; 

	$tot_allv = sprintf("%.3f",$tot_al) + $fudge2;
   	 
	$hbond1 = $$fh32{$tm2a} + $$fh34{$tm4a} + $$fh35{$tm5a} + $$fh36{$tm6a} + $$fh37{$tm7a} + $$fh71{$tm1a} + $$fh72{$tm2a} + $$fh76{$tm6a} + $$fh12{$tm2a} + $$fh65{$tm5a} + $$fh24{$tm4a} + $$fh54{$tm4a};
	$hbond = sprintf("%.3f",$hbond1) + $fudge2;
	...
	if ($j eq $nostruc)
	{
		...
		@energyh = reverse sort { $a <=> $b } @energh;
		$max = $energy[0];
		$maxv = $energyv[0];
		$maxs = $energys[0];
		$maxh = $energyh[0];
	...
	if ($j > $nostruc && $hbond < $maxh)
        {
                ...
                $hbond{$strc} = $hbond;
                $struch{$hbond{$strc}} = $strc;
                delete $tot_allvh{$maxstrh};
                delete $tot_allh{$maxstrh};
                delete $tot_someh{$maxstrh};
                delete $hbond{$maxstrh};
                delete $struch{$maxh};
                @energh = values (%hbond);
                @energyh = reverse sort { $a <=> $b } @energh;
                $maxh = $energyh[0];
...
@energyxh = sort { $a <=> $b } @energyh;

$outx1 = $prefix . ".bihelix_${geten}_TotalE.out";
open (OUT1,">$outx1");
$outx2 = $prefix . ".bihelix_${geten}_NoInterHelVDW.out";
open (OUT2,">$outx2");
$outx3 = $prefix . ".bihelix_${geten}_InterHelE.out";
open (OUT3,">$outx3");
$outx5 = $prefix . ".bihelix_${geten}_HBond.out";
open (OUT5,">$outx5"); 

if ($spe)
{
$outx4 = $prefix . ".bihelix_${geten}_specific.out";
open (OUT4,">$outx4");
}

print OUT1 "                                    Ordered by\n";
print OUT1 "                    Structure         TotalE     NoInterHelVDW    InterHelE        InterHB\n";

print OUT2 "                                                  Ordered by\n";
print OUT2 "                    Structure         TotalE     NoInterHelVDW    InterHelE        InterHB\n";

print OUT3 "                                                                  Ordered by\n";
print OUT3 "                    Structure         TotalE     NoInterHelVDW    InterHelE        InterHB\n";

print OUT5 "                                                                                 Ordered by\n";
print OUT5 "                    Structure         TotalE     NoInterHelVDW    InterHelE        InterHB\n";

if ($spe)
{
print OUT4 "                    Structure         TotalE     NoInterHelVDW    InterHelE        InterHB\n";
}
...
foreach $energyx (@energyx)
{
	...
	$hbondx = sprintf("%.1f",$hbondx{$struc{$energyx}});
...
foreach $energyxh (@energyxh)
{
        $hbondhh = sprintf("%.1f",$energyxh);
        $energyhv = sprintf("%.1f",$tot_allvh{$struch{$energyxh}});
        $energyhx = sprintf("%.1f",$tot_allh{$struch{$energyxh}});
        $energyhs = sprintf("%.1f",$tot_someh{$struch{$energyxh}});
        printf OUT5 "%29s %14.1f %14.1f %14.1f %14.1f\n",$struch{$energyxh},$energyhx,$energyhs,$energyhv,$hbondhh;
}

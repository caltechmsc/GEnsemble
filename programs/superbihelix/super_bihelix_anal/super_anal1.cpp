...
main (int argc, char* argv[])
{
  ...
  int total;
  numb = numbx;
  total = (phi_num[1])*(thet_num[1])*(hpm_num[1])*(eta_num[1])*(phi_num[2])*(thet_num[2])*(hpm_num[2])*(eta_num[2])*(phi_num[3])*(thet_num[3])*(hpm_num[3])*(eta_num[3])*(phi_num[7])*(thet_num[7])*(hpm_num[7])*(eta_num[
7]);
  if (total < numbx)
  {numb = total;}
  numb1 = numb - 1;

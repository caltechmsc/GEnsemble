...
if ($bgf eq "")
{die "run_super_bihelixLTP.pl

Jenelle Bray (jenelle@wag), August 4, 2009
Modified Ravi Abrol (abrol@wag), Sept 20, 2014 for using local theta phi (LTP) coordinate system
...
Program: /project/Biogroup/Software/GEnsemble/programs/superbihelix/run_super_bihelixLTP.pl

Usage: run_super_bihelixLTP.pl {bgf file} {mfta file} {number of ouput structures} {completeness level for analysis step}
...
anglesLTP.txt - The file with the changes in the eta, phi, theta angles to test, as well as different HPM centers to test. The anglesLTP.txt file should look like the file below for all seven helices (there should be same number of theta and phi grid points, as in the local theta phi space for each helix, theta and phi are coupled):

TM1 eta -30 -15 0 15 30
TM1 HPM 49.77
TM1 phi -30 -15 0 15 30
TM1 theta -20 -10 0 10 20
TM2 eta -30 -15 0 15 30 
TM2 HPM 84.04
TM2 phi -30 -15 0 15 30
TM2 theta -20 -10 0 10 20

Please make sure there are no extra spaces at the end of each line. For the eta, phi and theta values, these are the changes in the angles to make to the original bgf structure. So, if the input bgf file has TM1 theta of x degrees, the program will sample x, x-15, x+15, x-30 and x+30. For HPM, these are just the values of the HPM centers to test. If you are not sampling HPM centers, do not put the HPM centers in the anglesLTP.txt file. If you do not include a degree of freedom, the program will just keep it the same as the input file. For example, if you would just like to sample the eta values, like the old bihelix, your anglesLTP.txt file would be:
...
Example: run_super_bihelixLTP.pl beta2.bgf beta2.mfta 2500 2\n\n"}

if (! -e 'anglesLTP.txt')
{die "Could not find anglesLTP.txt \n"}
...
system ("/project/Biogroup/Software/GEnsemble/programs/superbihelix/fix_angles_fileLTP $bgf $mfta");
...
for ($i = 0; $i < 12; $i++)
{
  ...
  print PBS "/project/Biogroup/Software/GEnsemble/programs/superbihelix/super_bihelixLTP_pl.pl $bgf $mfta $tm1[$i] $tm2[$i] $nm $lst > \$SCRATCH/\${PBS_JOBNAME}.output\n\n";

#
# myMath::VectorReal     Vector Mathematics 
#
#
...
#
package myMath::VectorReal;

=head1 NAME

myMath::VectorReal - Module to handle 3D Vector Mathematics

=head1 SYNOPSIS

    #!/usr/bin/perl
    use myMath::VectorReal;
    ...
    $myMath::VectorReal::FORMAT = "[ %.5f %.5f %.5f ]";
    print "Modified default output format   \$a => $a\n";

    print 'length     => ', $a->length, "\n";
    print 'normalised => ', $a->norm, "\n";

    use myMath::VectorReal qw(:all);  # Include O X Y Z axis constant vectors
    ...
    use myMath::MatrixReal;  # Not required for pure vector math as above
...
The C<myMath::VectorReal> package defines a 3D mathematical "vector", in a way
that is compatible with the previous CPAN module C<myMath::MatrixReal>. However
...
While this package is compatible with myMath::MatrixReal, you DO NOT need to
...
The vectors are defined in the same way as a "row" C<myMath::MatrixReal> matrix,
...
between this package and that of myMath::MatrixReal, (the left objects overload
...
matrices from C<myMath::VectorReal> objects.  See a rough example in the
synopsis above, or in the file "matrix_test" in the packages source.

NOTE: the 6th element the C<myMath::MatrixReal> array object is used to hold the
...
these objects in the C<myMath::MatrixReal> functions.

=cut

use strict;
#require myMath::MatrixReal;  # not required!
...
# The format is as per a myMath::MatrixReal object, with extra length item
...
reference to create a C<myMath::VectorReal> object.

=item clone()

Return a completely new copy of the referring C<myMath::VectorReal> object.
...
By default this format is the same as it would be for a C<myMath::MatrixReal>
object, "[ %#19.12E %#19.12E %#19.12E ]\n".  Note that this includes a newline
character!.

However unlike C<myMath::MatrixReal> you can assign a new default sprintf
format by assigning it to the packages C<$FORMAT> variable. For Example

   $myMath::VectorReal::FORMAT = "{ %g, %g, %g }"
...
(row wise) to create a C<myMath::MatrixReal> translation matrix. For example
...
sub vector_matrix {
  ...
  bless [ [[$nx->array],
           [$ny->array],
           [$nz->array]], 3, 3 ],  "myMath::MatrixReal";
}

# ------------------------------------------------------------------
# Convertsions between myMath::MatrixReal and myMath::VectorReal packages

=back

=head1 VECTOR/MATRIX CONVERSION

The following functions provide links between the C<myMath::VectorReal> and
C<myMath::MatrixReal> packages.

NOTE: While this package is closely related to C<myMath::MatrixReal>, it does
...
Convert C<myMath::VectorReal> objects to a C<myMath::MatrixReal> objects.
Optional argument defines the object class to be returned (defaults to
C<myMath::MatrixReal>).

Note that as a C<myMath::VectorReal> is internally equivelent to a
C<myMath::MatrixReal> row matrix, C<vector2matrix_row> is essentually just a
bless operation, which is NOT required to use with C<myMath::MatrixReal>
functions.

The C<vector2matrix_col> performs the required transpose to convert the
C<myMath::VectorReal> object into a C<myMath::MatrixReal> version of a vector (a
column matrix).
...
When referred to by a C<myMath::MatrixReal> object, extracts the vector
from the matrix. the optional argument defines which row or column of the
matrix is to be extracted as a C<myMath::VectorReal> vector.

=cut

{  # Enclose MartixReal package in a block
package myMath::MatrixReal; # Fake a change into the myMath::MatrixReal package
...
sub matrix_row2vector {
  ...
  else {    # if no option, it must be a myMath::MatrixReal Row Vector
    croak "Error: matrix given to matrix_row2vector is not a 3D row matrix"
           unless ($rows == 1);
    $r = 0;
  }
  return myMath::VectorReal->new(@{$m->[0][$r]}); # same result, only cleaned up
}
...
sub matrix_col2vector {
  ...
  else {    # if no option, it must be a myMath::MatrixReal Column Vector
    croak "Error: matrix given to matrix_col2vector is not a 3D column matrix"
           unless ($cols == 1);
    $c = 0;
  }
  return myMath::VectorReal->new($m->[0][0][$c], $m->[0][1][$c], $m->[0][2][$c]);
}

} # Return to the myMath::VectorReal package we are really defining
...
sub _multiply {
  ...
  _trace("'*'",$object,$argument,$flip);
  if ( ref($argument) ) {
    # Assume multiply by  myMath::MatrixReal object  EG:  $v * $M --> $new_v
...
The C<myMath::MatrixReal> CPAN Module by   Steffen Beyer
and the C<myMath::MatrixReal-Ext1> CPAN extension by  Mike South

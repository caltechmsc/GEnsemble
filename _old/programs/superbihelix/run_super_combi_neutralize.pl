...
- neut1 = Neutral-Prep-06-16-09.pl (Victor's previous default)
- neut2 = Neutral-Prep/Neutral-Prep.pl (old default, may crash for some cases)
- neut3 = DEFAULT /project/Biogroup/Software/Neutralize/neutralize.py (new Vaclav's code)
...
Example:  run_super_combi_neutralize.pl -b beta2.bgf -m beta2.mfta -n 1000 -p 10
\n\n"}
...
if (not defined $neut){$neut = "neut3"};
...
### added ###
if (not defined $totin){$totin = "Super_Bihelix_InterHelE.out";}
open(TOTIN,"<$totin");
@totin = <TOTIN>;
close(TOTIN);
...
for ($i = 1; $i <= $proc; $i++)
{
  ...
  if (defined $debug) {
     print PBS "/project/Biogroup/Software/GEnsemble/programs/superbihelix/super_combi_neutralize.pl -b $bgf -m $mfta -n1 $num1 -n2 $num2 -p $i -f $fin -r $ref --bio $bio -md $masterdir --mach $machine -n $neut \n\n";
  } else {
     print PBS "/project/Biogroup/Software/GEnsemble/programs/superbihelix/super_combi_neutralize_new.pl -b $bgf -m $mfta -n1 $num1 -n2 $num2 -p $i -f $fin -r $ref --bio $bio -md $masterdir --mach $machine -n $neut \n\n";
  }

...
foreach $totinline (@totin)
{
  ...
  if ($i >= $num_str1 && $i <= $num_str2)  
  {
    ...
    system ("rm -f Residue-E.txt best_1.bgf best_1.min.bgf $bundle Anneal-Energies.txt mpsim.out scream.par scream.out.gz timing.txt $template $tempnew  Field1.bgf.gz");
  }
}
...
foreach $templine (@template)
{
  ...
  if ($templine !~ /EtaRes/)
  {
    ...
    ($sp,$rmsdx) = split /\ /, $rmsd[0];
    print OUT ("$finbgf  $hbond  $totscream  $premin  $postmin  $rmsdx");
    system ("rm -f Residue-E.txt best_1.bgf best_1.min.bgf $bundle Anneal-Energies.txt mpsim.out scream.par scream.out.gz timing.txt $template $tempnew $pdb Field1.bgf.gz");
}
...
if ($fin == 1)
{
  ...
  system("/project/Biogroup/Software/GEnsemble/programs/superbihelix/super_combi_sort_bartosz.pl $proc $bgf $tag"); 
}

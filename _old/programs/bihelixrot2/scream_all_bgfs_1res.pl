...
for ($i=1;$i<=100;$i++) {
system("cp -f ../top100/wt$i-*bgf wt$i.bgf");
}
@bgfl = `ls *.bgf`;
chomp @bgfl;
...
#   system("$sbin/annex/add_GPCR_chains2.pl $bgfl > temp1.bgf");
    system("$sbin/../$python_executable $sbin/../$scream_dir/scripts/SCREAM_wrap.py $bgfl $rotadiv FULL 0.0 $mutate");
    $bgfls = $bgfp."-".$label.'.bgf';
    system("cp -f best_1.bgf $bgfls");
    if (defined $mpsim) {
    $bgfme = $bgfp."-".$label.'_mpsimE.out';

########################################################################
# $Header: /var/local/cvsroot/4Suite/Ft/Xml/XPath/ParsedExpr.py,v 1.45 2005/08/02 22:43:00 mbrown Exp $
"""
...
def _nodeset_compare(compare, a, b, relational=False):
    ...
    if isinstance(a, Types.NodesetType) and isinstance(b, Types.NodesetType):
        ...
        # is true.
        #
        # (Note, however, that relational comparisons convert their operands
        # to numbers.)
        if relational:
            if not(a and b):
                # relational, and at least one of the node-sets is empty
                return boolean.false
            else:
                # relational, and neither node-set is empty
                coerce = Conversions.NumberValue
        elif not(a and b):
            # not relational, and at least one of the node-sets is empty
            return compare(a, b) and boolean.true or boolean.false
        else:
            # not relational, and neither node-set is empty
            coerce = Conversions.StringValue

        # convert the larger set to strings or numbers one time,
        # then iterate over the smaller set, comparing to each item in
        # the converted larger set.
        if len(a) > len(b):
            coerced = [coerce(node) for node in a]
            nodes = b
        else:
            coerced = [coerce(node) for node in b]
            nodes = a
        for node in nodes:
            current = coerce(node)
    ...
    if isinstance(a, Types.NodesetType):
        ...
        if isinstance(b, Types.BooleanType):
            return compare(Conversions.BooleanValue(a), b) \
        ...
        if isinstance(a, Types.BooleanType):
            return compare(Conversions.BooleanValue(b), a) \
                   and boolean.true or boolean.false
        elif relational:
...
class ParsedEqualityExpr:
    ...
    def evaluate(self, context):
        ...
        elif isinstance(left, Types.NumberType) or \
             isinstance(right, Types.NumberType):
            coerce = Conversions.NumberValue
        else:
            coerce = Conversions.StringValue

        result = self._cmp(coerce(left), coerce(right))
        return result and boolean.true or boolean.false
...
class ParsedRelationalExpr:
    ...
    def evaluate(self, context):
        ...
        a = Conversions.NumberValue(left)
        b = Conversions.NumberValue(right)
        return self._cmp(a, b) and boolean.true or boolean.false

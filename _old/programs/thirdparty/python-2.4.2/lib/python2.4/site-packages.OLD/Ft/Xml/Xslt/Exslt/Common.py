########################################################################
# $Header: /var/local/cvsroot/4Suite/Ft/Xml/Xslt/Exslt/Common.py,v 1.14 2005/10/19 15:03:00 uogbuji Exp $
"""
...
def ObjectType(context, obj):
    ...
    if type(obj) is type([]):
        ...
        return u'number'
    elif type(obj) is type(boolean.true):
        return u'boolean'
    elif getattr(obj, 'nodeType', None) == Node.DOCUMENT_NODE:
        return u'RTF'
    else:
...
class DocumentElement(XsltElement):
    ...
    def instantiate(self, context, processor):
        ...
        #Use the main input source for relative URI rather than the main
        #"output source"
        uri = processor._documentInputSource._normalize(href)
        ...
        if (self.attributes.get((FT_EXT_NAMESPACE, 'overwrite-safeguard') == u'yes')
            ...
            #FIXME: Security hole.  Switch to the mkstemp as soon as we mandate Python 2.3 mnimum
            savefile = tempfile.mktemp('', os.path.split(path)[-1]+'-')
            processor.warn("The file you are trying to create with exsl:document already exists.  As a safety measure it will be copied to a temporary file '%s'."%savefile)  #FIXME: l10n
            try:
                shutil.copyfile(path, savefile)
            except:
                raise XsltRuntimeException(ExsltError.ABORTED_EXSLDOCUMENT_OVERWRITE,

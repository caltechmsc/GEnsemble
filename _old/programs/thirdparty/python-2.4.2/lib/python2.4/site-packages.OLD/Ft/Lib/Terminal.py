########################################################################
# $Header: /var/local/cvsroot/4Suite/Ft/Lib/Terminal.py,v 1.5 2005/08/26 22:32:09 jkloth Exp $
"""
...
# Methods a Terminal instance should expose from its underly stream.
_file_methods = ('flush', 'write', 'read', 'isatty')
...
class Terminal:
    ...
    def _init_win32(self, stream, keepAnsiEscapes):
        try:
            self._handle = h = msvcrt.get_osfhandle(stream.fileno())
        except IOError:
            pass
        else:
            if keepAnsiEscapes:

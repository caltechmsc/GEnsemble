...
"""
 ...
 The accompanying crys module has essentially the same functionality,
  but is written in C for greater speed.
    ...
    if X < 3.e-7:
        if n == 1:
            ...
            return ([1.30693606237085E-01 -2.90430236082028E-02 *X,
                    ...
                    [6.52145154862545E-01 -1.22713621927067E-01 *X,
                    3.47854845137453E-01 -2.10619711404725E-01 *X])
        elif n == 3:
            return ([6.03769246832797E-02 -9.28875764357368E-03 *X,
                     7.76823355931043E-01 -1.19511285527878E-01 *X,
                     6.66279971938567E+00 -1.02504611068957E+00 *X],
...
def Root4(X):
    ...
    if X <= 3.0E-7:
        ...
        WW2 = W24*WW1
        WW1 = WW1-WW2-WW3-WW4
    return [RT1,RT2,RT3,RT4],[WW1,WW2,WW3,WW4]
...
def RecurFactors(t,A,B,Px,Qx,xi,xk):
    ...
    B1 = (1-B*fact)/(2*A)         
    B1p = (1-A*fact)/(2*B)        
    C = (Px-xi) + B*(Qx-Px)*fact  
    Cp = (Qx-xk) + A*(Px-Qx)*fact
    return C,Cp,B0,B1,B1p

def RecurFactorsGamess(t,A,B,Px,Qx,xi,xk):
    # Analogous versions taken from Gamess source code
    fact = t/(A+B)/(1+t)
    B0 = 0.5*fact
    B1 = 1/(2*A*(1+t)) + 0.5*fact
    B1p = 1/(2*B*(1+t)) + 0.5*fact
    C = (Px-xi)/(1+t) + (B*(Qx-xi)+A*(Px-xi))*fact
    Cp = (Qx-xk)/(1+t) + (B*(Qx-xk)+A*(Px-xk))*fact
    return C,Cp,B0,B1,B1p

def Recur(t,i,j,k,l,xi,xj,xk,xl,alphai,alphaj,alphak,alphal):
    ...
    G = zeros((n+1,m+1),Float)

    C,Cp,B0,B1,B1p = RecurFactorsGamess(t,A,B,Px,Qx,xi,xk)

    # ABD eq 11.
    G[0,0] = pi*exp(-alphai*alphaj*pow(xi-xj,2)/(alphai+alphaj)

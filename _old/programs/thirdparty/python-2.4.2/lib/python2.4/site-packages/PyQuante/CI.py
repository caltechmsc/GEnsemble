...
from Numeric import zeros,Float,dot,matrixmultiply
...
def CISMatrix(Ints,orbs,Ehf,orbe,occs):
    ...
    print "Excitations to be calculated: ",singles
    nex = len(singles)

    MOInts = TransformInts(Ints,orbs)

    # see Szabo/Ostlund Table 4.1
    CIMatrix = zeros((nex,nex),Float)
    for ar in range(nex):
        a,r = singles[ar]
        for bs in range(nex):
            b,s = singles[bs]
...
def CISDMatrix(Ints,orbs,Ehf,orbe,occs):
    ...
    doubles = []
    print "Excitations to be calculated: ",singles+doubles
    nsin = len(singles)
    ndoub = len(doubles)
    nex = nsin+ndoub
...
def TransformInts(Ints,orbs):
    ...
    temp = zeros((nbf,nbf,nbf,nmo),Float)
    tempvec = zeros(nbf,Float)
    temp2 = zeros((nbf,nbf,nmo,nmo),Float)

...
from Numeric import zeros,Float,sum,dot
...
def TransformIntsMP2(Ints,orbs,nclosed):
    ...
    temp = zeros((nbf,nbf,nclosed,nbf),Float)
    ...
    temp2 = zeros((nclosed,nbf,nclosed,nbf),Float)
    for nu in bfs:
        for eta in bfs:
            for b in occs:
                for a in occs:
    ...
    temp = zeros((nclosed,nbf,nclosed,nmo),Float)
    for a in occs:
        for nu in bfs:
            for b in occs:
                for j in mos:

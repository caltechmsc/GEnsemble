...
class VelocityVerlet_f(Integrator):
    ...
    def __init__(self, molName="cmdf", timeStep=0.1, translateVector=zeros([3], Float),  \
                imove=zeros([1], Int), masses=ones([1], Float)):
        Integrator.__init__(self, timeStep) 
        self.translateVector = translateVector
        self.imove = imove
        self.masses = masses
    ...
    def run(self, forceEngine, r, v, f):
        ...
        masses = self.masses
        rold = r.copy()
        r, rold, v = integrator_f.verlet1(dt, trans, imove, masses, r, rold, v, f)
        forceEngine.setCoords(r)
        forceEngine.calcEnergyForce()
        f = forceEngine.getForces()
        energy = forceEngine.getEnergy()
        v = integrator_f.verlet2(dt, imove, masses, v, f)

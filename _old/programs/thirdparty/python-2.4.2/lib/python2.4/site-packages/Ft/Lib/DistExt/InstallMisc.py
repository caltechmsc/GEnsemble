...
class InstallMisc(Command):
    ...
    def get_allfiles(self):
        if self.allfiles is None:
            # Determine the complete set of files possible
            allfiles = FileList()
            allfiles.findall()
            ...
            allfiles.exclude_pattern(r'(\%s)?CVS(\.sandboxinfo)?\%s' %
                                     (os.sep, os.sep), is_regex=True)
            allfiles.exclude_pattern(r'\.cvsignore$', is_regex=True)
            allfiles.exclude_pattern(r'(\.)?#[^\%s]+#$' % os.sep,
                                     is_regex=True)
            allfiles.exclude_pattern(r'\.py[co]$', is_regex=True)

            # Save the resulting file list for later reuse
            self.allfiles = allfiles.files

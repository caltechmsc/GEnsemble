...
#                                                                                                       # 
# Created on 06/20/06, last revision on 07/03/06. yi                                                    #
#                                                                                                       # 
#########################################################################################################
...
import scipy
from scipy.optimize.optimize import fmin, fmin_bfgs, fmin_cg, fmin_ncg 
#from cmdf.utility.control import Control
#import cmdf.mee.mee as mee
#from cmdf.dynamics.meeForceEngine import MeeForceEngine
#from cmdf.dynamics.reaxForceEngine import ReaxForceEngine
#from cmdf.utility.communication import systemToArray, arraysToSystem_md, arraysToSystem_min
import cmdf.utility.communication as communication 
...
class Simplex(Minimize):
    ...
    def run(self, system, forceEngine):
        ...
        f = forceEngine.calcEnergy
        fprime = forceEngine.calcGradient
        f0 = f(x0)
        g0 = fprime(x0)
        if printLevel > 0:
        ...
        minout = fmin(f, x0, self.args, self.xthreshold, self.fthreshold, self.maxiteration, self.maxfunction, self.full_output, self.disp, self.retall)

        energy = minout[1]                
        coords = minout[0]
        energy_final = f(coords) 
        force_final = fprime(coords) 
        communication.arraysToSystem_min(system, coords, energy_final, force_final)  # Update system information.
...
class BFGS(Minimize):
    ...
    def run(self, system, forceEngine):
        ...
        minout = fmin_bfgs(f, x0, fprime, self.args, self.gthreshold, self.norm, self.epsilon, self.maxiteration, self.full_output, self.disp, self.retall)
...
class ConjugateGradient_PR(Minimize):
    ...
    def run(self, system, forceEngine):
        ...
        minout = fmin_cg(f, x0, fprime, self.args, self.gthreshold, self.norm, self.epsilon, self.maxiteration, self.full_output, self.disp, self.retall)
...
class Newton_CG(Minimize):
    ...
    ### Warning: Coordinates don't update due to a possible bug. 
        ...
        minout = fmin_ncg(f, x0, fprime, fhess_p, fhess, self.args, self.xthreshold, self.epsilon, self.maxiteration, self.full_output, self.disp, self.retall)
...
class Powell(Minimize):
    ...
    def run(self, system, forceEngine):
        ...
        minout = fmin_powell(f, x0, self.args, self.xthreshold, self.fthreshold, self.maxiteration, self.maxfunction, self.full_output, self.disp, self.retall)

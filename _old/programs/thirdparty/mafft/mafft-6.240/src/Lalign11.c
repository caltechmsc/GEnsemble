...
static void match_calcLN( float *match, char **s1, char **s2, int i1, int lgth2 ) 
{
	char *seq2 = s2[0];
	int *intptr;

	intptr = amino_disLN[s1[0][i1]];
	while( lgth2-- )
		*match++ = intptr[*seq2++];
}
#else
...
float L__align11_noalign( char **seq1, char **seq2, int alloclen, int *off1pt, int *off2pt, int saimatch )
/* score no keisan no sai motokaraaru gap no atukai ni mondai ga aru */
{
//	int k;
	register int i, j;
...
//	float localthr = 100;
//	float localthr2 = 100;
	float fpenalty = (float)penaltyLN;
	float fpenalty_ex = (float)penalty_exLN;
...
//	ijp = commonIP;
	...
	match_calcLN( initverticalw, seq2, seq1, 0, lgth1 );

	match_calcLN( currentw, seq1, seq2, 0, lgth2 );
...
fprintf( stderr, "\n" );
	...
	for( i=1; i<lasti; i++ )
	{
		...
		match_calcLN( currentw, seq1, seq2, i, lgth2 );
#if DEBUG2
		fprintf( stderr, "%c   ", seq1[0][i] );
		fprintf( stderr, "%5.0f ", currentw[0] );
#endif

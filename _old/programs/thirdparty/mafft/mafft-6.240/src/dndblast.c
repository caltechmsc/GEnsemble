...
//		fprintf( stderr, "###  i = %d\n", i );
		...
		if( scoremtx == -1 )
			sprintf( com, "blastall -e 1e10 -p blastn -m 7  -i %s -d %s >  %s\0", queryfile, datafile, fastafile );
		else
			sprintf( com, "blastall -G 10 -E 1 -e 1e10 -p blastp -m 7  -i %s -d %s >  %s\0", queryfile, datafile, fastafile );
		res = system( com );
...
//		nlen1[0] = nlen[i];
		...
		if( scoremtx == -1 )
			sprintf( com, "fasta34 -z3 -m10  -n -Q  -b%d -E%d -d%d %s %s %d > %s\0", M, M, 0, queryfile, datafile, ktuple, fastafile );
		else
			sprintf( com, "fasta34 -z3 -m10  -Q  -b%d -E%d -d%d %s %s %d > %s\0", M, M, 0, queryfile, datafile, ktuple, fastafile );
		res = system( com );
		if( res ) ErrorExit( "error in fasta" );

...
void treebase( int nlen[M], char **aseq, char **mseq1, char **mseq2, int ***topol, double *effarr, int *alloclen, LocalHom **localhomtable )
{
	int i, j, l;
	int len1, len2;
	int clus1, clus2;
...
//	writePre( njob, name, nlen, seq, 0 );
	...
	if( tbutree == 0 )
	{
		...
		for( i=0; i<ien; i++ ) 
		{
			...
			for( j=i+1; j<njob; j++ ) 
			{
//				iscore[i][j-i] = (float)( substitution_hosei( seq[i], seq[j] ) );
				iscore[i][j-i] = 1.0 - (float)naivepairscore11( seq[i], seq[j], penalty ) / MIN( selfscore[i], selfscore[j] );
			}
		}
		fprintf( stderr, "\ndone.\n\n" );
#endif
	...
	WriteHat2_floathalf( prep, njob, name, iscore ); // name chuui
	fclose( prep );
	exit( 1 );
#endif
...
//	countnode( njob, topol, node0 );
	...
	treebase( nlen, bseq, mseq1, mseq2, topol, eff, &alloclen, localhomtable );
	fprintf( stderr, "\ndone.\n" );

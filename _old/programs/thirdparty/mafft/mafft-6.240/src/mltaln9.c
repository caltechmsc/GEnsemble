...
char seqcheck( char **seq )
{
	...
	while( *seq )	
	{
		...
		for( i=0; i<len; i++ ) 
		{
			...
			{
				fprintf( stderr, "Error in site %d in seq = %s\n", i, *seq );
				return( (int)(*seq)[i] );
			}
		}
		seq++;
...
void fixed_musclesupg_float_realloc_nobk_halfmtx_treeout( int nseq, float **eff, int ***topol, float **len, char **name, int *nlen )
{
    ...
    for( i=0; i<nseq; i++ )
	{
		...
		nametmp[10] = 0;
		sprintf( tree[i], "%d_l=%d%.5s", i+1, nlen[i], nametmp );
	}
	for( i=0; i<nseq; i++ )
	{
		ac[i].next = ac+i+1;
...
void veryfastupg_float_realloc_nobk_halfmtx( int nseq, float **eff, int ***topol, float **len )
{
	int i, j, k, miniim, maxiim, minijm, maxijm;
	int *intpt, *intpt2;
	float tmpfloat;
...
void veryfastspg_float_realloc_nobk_halfmtx( int nseq, float **eff, int ***topol, float **len )
{
	int i, j, k, miniim, maxiim, minijm, maxijm;
	int *intpt, *intpt2;
	float tmpfloat;
...
void veryfastsupg_float_realloc_nobk_halfmtx( int nseq, float **eff, int ***topol, float **len )
{
	int i, j, k, miniim, maxiim, minijm, maxijm;
	int *intpt, *intpt2;
	float tmpfloat;
...
void veryfastsupg_float_realloc_nobk( int nseq, float **eff, int ***topol, float **len )
{
	int i, j, k, miniim, maxiim, minijm, maxijm;
	int *intpt, *intpt2;
	float tmpfloat;
...
//		fprintf( stdout, "im=%d, jm=%d, minscore = %f\n", im, jm, minscore );
...
void veryfastsupg_int_realloc_nobk( int nseq, int **eff, int ***topol, double **len )
{
	int i, j, k, miniim, maxiim, minijm, maxijm;
	int *intpt, *intpt2;
	int tmpint;
...
//		fprintf( stderr, "im=%d, jm=%d\n", im, jm );
		...
		for( i=0; i!=-1; i=ac[i].next )
        {
            if( i != im && i != jm )
            {
				...
				eff1 = eff[minijm][maxijm];
                eff[miniim][maxiim] =
				(int) ( (float)MIN( eff0, eff1 ) * ( 1.0 - SUEFF ) + (float)( eff0 + eff1 ) * 0.5 * SUEFF + 0.5 ); // ??????? 060410
            }
        }
		acjmprev = ac[jm].prev; 
		acjmnext = ac[jm].next; 
...
#endif
void veryfastsupg_float_realloc_nobk_halfmtx_treeout( int nseq, float **eff, int ***topol, float **len )
{
	int i, j, k, miniim, maxiim, minijm, maxijm;
	int *intpt, *intpt2;
	float tmpfloat;
	...
	if( !hist )
	{
		treetmp = AllocateCharVec( njob*10 );
		tree = AllocateCharMtx( njob, njob*10 );
		hist = AllocateIntVec( njob );
		tmptmplen = AllocateFloatVec( njob );
		ac = (Bchain *)malloc( njob * sizeof( Bchain ) );
		nmemar = AllocateIntVec( njob );
...
void veryfastsupg_double_outtree( int nseq, double **eff, int ***topol, double **len )
{
    int i, j, k, miniim, maxiim, minijm, maxijm;
	int *intpt, *intpt2;
	double tmpdouble;
...
//		if( pv ) fprintf( stderr, "Penal!, %f, %d-%d, pos1,pos2=%d,%d\n", pv * deff * 0.5,  i, j, p1-seq1[i], p2-seq2[j] );
		p1p = p1; p2p = p2;
		valf += (float)amino_dis[*p1++][*p2++] + 0.5 * pv;
		while( *p1p )
		{
			pv = 0;
			if( *p1p != '-' && *p2p != '-' )
...
//		fprintf( stderr, "penal = %d\n", penal );
		...
		if( *p1 != '-' && *p2 == '-' )
			pv = penal;
		if( pv ) fprintf( stderr, "Penal!, %f, %d-%d, pos1,pos2=%d,%d\n", pv * feff * 0.5,  i, j, p1-seq1[i], p2-seq2[j] );
		p1p = p1; p2p = p2;
		valf += (float)amino_dis[*p1++][*p2++] + 0.5 * pv;
		while( *p1p )
		{

...
##          vmd@ks.uiuc.edu
##
## $Id: autopsf.tcl,v 1.102 2007/02/22 00:25:40 petefred Exp $
##
## Home Page:
...
#Frame for patches
  ...
  grid [frame $w.extras] -row $row -column 0 -columnspan 4 -sticky nsew
  label $w.extras.label -text "Extras:" -width 8
  checkbutton $w.extras.water -text "Solvate" -width 8 -variable [namespace current]::water
  checkbutton $w.extras.ionize -text "Ionize" -width 7 -variable [namespace current]::ionize
  checkbutton $w.extras.regen -text "Regen" -width 8 -variable [namespace current]::regenall
  pack $w.extras.label $w.extras.water $w.extras.ionize $w.extras.regen -side  left -fill x -expand 1
 # grid 
  incr row
...
#puts "$curres $oldres $residues"
             incr residues
#             puts "residues: $residues"
	     lappend segtoseg   [list [list $curseg $curres] $segid]
             set oldres $curres
	  }
...
proc ::autopsf::find_ssbonds { {dist 3.0} } {
   ...
   foreach sg [$sel get {index segid chain resid}] {
      foreach { index segid chain resid } $sg { break } 
      set segkey chain
      set segment $chain
      if {![llength $chain]} { 
	 set segkey segid
	 set segment $segid
      ...
      foreach sg2 [$sel2 get {segid chain resid}] {
	 foreach { segid2 chain2 resid2 } $sg2 { break }
	 set segkey2 chain
	 set segment2 $chain2
	 if {![llength $chain2]} { 
	    set segkey2 segid

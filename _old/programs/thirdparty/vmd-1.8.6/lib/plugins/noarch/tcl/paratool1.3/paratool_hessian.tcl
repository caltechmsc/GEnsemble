...
proc ::Paratool::Hessian::compute_force_constants_from_inthessian { args } {
   ...
   for {set i 1} {$i<[llength $zmat]} {incr i} {
	 ...
	 $sel delete
      ...
      # mutually dependent. We can then use ::QMtool::measure_$type to determine how much
	 ...
	 set xmeasure [::QMtool::measure_[lindex $zmat $j 1] $molidbase [lindex $zmat $j 2] \
			  [list $icenter $iupper]]

	 #foreach {xcenter xlower xupper} [lrange $xmeasure end-2 end] {break}
	 foreach {xcenter xupper} $xmeasure {break}
	 if {[regexp "angle|lbend|dihed" [lindex $zmat $j 1]]} {
	 ...
	 set xmeasure [::QMtool::measure_[lindex $zmat $rdep 1] $molidbase [lindex $zmat $rdep 2] \
			  [list $icenter $iupper]]
	 #foreach {xcenter xlower xupper} [lrange $xmeasure end-2 end] {break}
	 foreach {xcenter xupper} $xmeasure {break}
	 if {$xcenter<-90} { set xcenter [expr {$xcenter+360}] }
	 ...
	 if {[regexp "angle|lbend|dihed" [lindex $zmat $rdep 1]]} {
	 ...
	 foreach cdep $depcoor {
	    ...
	    if {$kc==0.0} { continue }
	    set xmeasure [::QMtool::measure_[lindex $zmat $cdep 1] $molidbase [lindex $zmat $cdep 2] [list $icenter $iupper]]
	    #foreach {xcenter xlower xupper} [lrange $xmeasure end-2 end] {break}
	    foreach {xcenter xupper} $xmeasure {break}
	    if {$xcenter<-90} { set xcenter [expr {$xcenter+360}] }
	    if {$xupper<-90}  { set xupper  [expr {$xupper+360}] }
	    set h2 [expr {($xupper-$xcenter)}]
	    if {[regexp "angle|lbend|dihed" [lindex $zmat $cdep 1]]} {

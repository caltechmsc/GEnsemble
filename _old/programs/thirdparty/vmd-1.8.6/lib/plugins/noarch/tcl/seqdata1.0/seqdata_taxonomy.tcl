...
namespace eval ::SeqData::Taxonomy {
    ...
    proc getLineage {taxonomyNode {showHidden 0} {includeRanks 0}} {
        ...
        if {[info exists nodeMap($taxonomyNode,parent)]} {
            ...
            # Construct the lineage starting with the current node.
            set parentNode $nodeMap($taxonomyNode,parent)
            ...
            if {$datadir != "" && [file exists [set filename [file join $datadir "names.dmp"]]]} {
                ...
                while {![eof $fp]} {
                    ...
                    if {[llength $fields] >= 4} {
                        ...
                        if {$type == "scientific name" || $type == "synonym" || $type == "genbank synonym"} {

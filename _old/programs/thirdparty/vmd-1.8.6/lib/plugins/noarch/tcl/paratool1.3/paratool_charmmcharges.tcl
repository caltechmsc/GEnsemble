...
#
# $Id: paratool_charmmcharges.tcl,v 1.21 2007/03/02 12:22:29 saam Exp $
#
...
proc ::CHARMMcharge::setup_charmm_charges {} {
   ...
   frame $v.group.list
   label $v.group.list.format -text "Natoms  Initial   Supram Center" -font {tkFixed 9}
   scrollbar $v.group.list.scroll -command "$v.group.list.list yview"
   listbox $v.group.list.list -activestyle dotbox -yscroll "$v.group.list.scroll set" -font {tkFixed 9} \
   ...
   $v.group.init.type configure -font {tkFixed 9}
   ...
   frame $v.sel.list
   label $v.sel.list.format -text "Index  Name  Initial   Supram Center" -font {tkFixed 9}
   scrollbar $v.sel.list.scroll -command "$v.sel.list.list yview"
   listbox $v.sel.list.list -activestyle none -yscroll "$v.sel.list.scroll set" -font {tkFixed 9} \
...
puts "[list $alphaposind $curcenter [join [$secondarysel list]] [join [$tertiarysel list]]]"
      set dihed [::QMtool::measure_dihed $molidsyswat [list [lindex [$asel list] 0] $curcenter [join [$secondarysel list]] [join [$tertiarysel list]]]]

...
#
# $Id: paratool_intcoor.tcl,v 1.12 2006/08/15 04:34:31 saam Exp $
#
proc ::Paratool::edit_internal_coords {} {
   ...
   label $v.zmat.pick.format -textvariable ::Paratool::intcoorformat \
      -relief flat -bd 2 -justify left -font {tkFixed 9}
   ...
   $v.zmat.pick.list configure -font {tkFixed 9}
...
proc ::Paratool::update_internal_coordinate_values {} {
   ...
   foreach entry $zmat {
	 ...
	 R* { lset zmat $i 3 [::QMtool::measure_bond  $molidbase [lindex $entry 2] last] }
	 A* { lset zmat $i 3 [::QMtool::measure_angle $molidbase [lindex $entry 2] last] }
	 D* { lset zmat $i 3 [::QMtool::measure_dihed $molidbase [lindex $entry 2] last] }
	 O* { lset zmat $i 3 [::QMtool::measure_imprp $molidbase [lindex $entry 2] last] }
      }
      incr i
   }
}
...
proc ::Paratool::find_linear_dependent_intcoor {izmatlist} {
   ...
   foreach izmat $izmatlist {
	 ...
	 A* {
	    ...
	    lappend dep [ldiff [lsearch -all -regexp $zmat $pattern1] [lsearch -all -regexp $zmat $pattern2]]
	 }
	 D* {
...
proc ::Paratool::remove_bond {atom0 atom1} {
   ...
   if {[lsearch $bondatom0 $atom1]>=0} {
      ...
      ::QMtool::vmd_delbond $atom0 $atom1 $molidbase
...
proc ::Paratool::register_bond {{indexlist {}}} {
   ...
   variable ringlist [find_rings $molidbase]
}
...
proc ::Paratool::bond_label { data } {
   ...
   lappend intcoormarktags [graphics $molidbase cylinder $pos1 $pos2 radius $radius]
...
proc ::Paratool::angle_label { data } {
   ...
   lappend intcoormarktags [graphics $molidbase cylinder $pos1 $pos2 radius $radius]
   lappend intcoormarktags [graphics $molidbase cylinder $pos2 $pos3 radius $radius]
   lappend intcoormarktags [graphics $molidbase sphere $pos2 radius $radius]
...
proc ::Paratool::dihed_label { data } {
   ...
   lappend intcoormarktags [graphics $molidbase cylinder $pos0 $pos1 radius $radius]
   lappend intcoormarktags [graphics $molidbase cylinder $pos1 $pos2 radius $radius]
   lappend intcoormarktags [graphics $molidbase cylinder $pos2 $pos3 radius $radius]
   lappend intcoormarktags [graphics $molidbase sphere $pos1 radius $radius]
   lappend intcoormarktags [graphics $molidbase sphere $pos2 radius $radius]
...
proc ::Paratool::del_coordinate {} {
   ...
   if {[llength $picklist]==2 && [lsearch $zmat "*bond {$picklist} *"]<0 && [lsearch $zmat "*bond {[lrevert $picklist]} *"]<0} {
      ...
      ::QMtool::vmd_delbond [lindex $picklist 0] [lindex $picklist 1] $molidbase
      update_intcoorlabels 
      return 0
   }
...
proc ::Paratool::edit_selected_parameters_gui {} {
   ...
   listbox $v.equiv.list.list -yscroll "$v.equiv.list.scroll set" -font {tkFixed 9} \
   ...
   label $v.edit.format -font {tkFixed 9} -textvariable ::Paratool::selpareditformat
...
proc ::Paratool::editparams_submit { {win {}} } {
   ...
   foreach selparam $selparamlist {
	 ...
	 lappend parlist $selectedpar3
      ...
      lset zmat $izmat 5 "[regsub {[QCRM]} [lindex $::Paratool::zmat $izmat 5] {}]M"
   }
   update_zmat
...
#   package require hesstrans
   ...
   set sel [atomselect $molidbase all]
   set inthess [hessiantransform [$sel get {x y z}] $hessian $bondlist $anglelist $dihedlist $imprplist]
...
proc ::Paratool::use_qm_parameters {} {
   ...
   foreach i $selintcoorlist {
	 ...
	 set pos [lsearch $zmatqm "* * {[lrevert [lindex $zmatentry 2]]} *"]
      ...
      lset zmat $izmat 5 "[regsub -all {[QCRM]} [lindex $zmatentry 5] {}]Q"      
   }
   update_zmat
...
proc ::Paratool::assign_unknown_force_constants_from_zmatqm {} {
   ...
   foreach zmatentry $zmat {
	 ...
	 lset zmat $i 5 "[regsub -all {[QCRM]} [lindex zmatentry 5] {}]Q"
      }
      incr i
   }
   if {$charmmoverridesqm} { 
 ...
 proc ::Paratool::toggle_exact_dihedrals { } {
    ...
    foreach entry $zmat {
	  ...
	  lset zmat [expr {$num+1}] 4 [list $fc $n $delta]
       ...
       lset zmat [expr {$num+1}] 5 "[regsub {[QCRM]} [lindex $zmat [expr {$num+1}] 5] {}]Q"
       incr num
    }
    variable havefc 1
    variable havepar 1

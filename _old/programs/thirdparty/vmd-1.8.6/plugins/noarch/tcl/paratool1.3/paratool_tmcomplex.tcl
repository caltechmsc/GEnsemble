...
# metal complex.
#
# $Id: paratool_tmcomplex.tcl,v 1.32 2006/08/15 04:34:31 saam Exp $
#
...
proc ::Paratool::metal_complex_gui {} {
   ...
   listbox $v.catoms.list.list -selectbackground $selectcolor -yscroll "$v.catoms.list.scroll set" \
      -width 55 -height 4 -setgrid 1 -selectmode browse -font {tkFixed 9} -activestyle none \
      -listvariable ::Paratool::formatcomplexlist
   pack $v.catoms.list.list $v.catoms.list.scroll -side left -fill y -expand 1
   label $v.catoms.total -font {tkFixed 9} -textvariable ::Paratool::centertotalcharge
   ...
   listbox $v.ligands.list.list -selectbackground $selectcolor -yscroll "$v.ligands.list.scroll set" \
      -width 55 -height 6 -setgrid 1 -selectmode browse -font {tkFixed 9} -activestyle none \
      -listvariable ::Paratool::formatligandlist
   pack $v.ligands.list.list $v.ligands.list.scroll -side left -fill y -expand 1

   label $v.ligands.total -font {tkFixed 9} -textvariable ::Paratool::ligandstotalcharge
...
proc ::Paratool::exclusions_gui {} {
   ...
   #label $w.types.format -font {tkFixed 9} -textvariable ::Paratool::exclusionsformat \
   #   -relief flat -bd 2 -justify left;

   frame $w.pairs.list
   
   scrollbar $w.pairs.list.scroll -command "$w.pairs.list.list yview" -takefocus 0
   listbox $w.pairs.list.list -yscroll "$w.pairs.list.scroll set" -font {tkFixed 9} \
...
proc ::Paratool::set_exclusion_policy {} {
   ...
   if {[string is integer $n]} {
	 ...
	 foreach atom [bondedsel $molidbase $center $center $n] {
	    if {$atom!=$center} {
      ...
      foreach ligand $tmligands {
	 set centroid [lindex $ligand 3]
	 foreach atom [bondedsel $molidbase $centroid $centroid $n] {
	    if {$atom!=$centroid} {

...
Output: The file that contains the energy terms is Bihelical-E.txt.
Format requirement:
Input bgf file must be a system with ONLY 2 chains--and they need to named chain A and chain B.

Overview of terms:
These are the terms that are outputted for each residue: Res  Intern  Tot_EL   EL_AA EL_AA_V EL_AA_C EL_AA_H   EL_AB EL_AB_V EL_AB_C EL_AB_H  Tot_Ss   Ss_AA Ss_AA_V Ss_AA_C Ss_AA_H   Ss_AB Ss_AB_V Ss_AB_C Ss_AB_H   Tot_E

"AA" and "AB" stands for "intra" and "inter" helical energy contributions.
Note that the inter-helical terms have already been divided by 2 from the energy you would otherwise get from a one-pt energy calculation, to get rid of double counting.

Detailed explanations:

Res: Residue name, or "All", "ChainA", "ChainB".
Intern: Internal energies of sidechains.
Tot_EL: Total empty lattice energy, also know as sc-fixed.
EL_AA: Intra-helical empty lattice energy.
EL_AA_V: VDW component of intra-helical empty lattice energy.
EL_AA_C: Coulomb component of intra-helical empty lattice energy.
EL_AA_H: HB component of intra-helical empty lattice energy.
EL_AB: Inter-helical empty lattice energy.
EL_AB_V: VDW component of inter-helical empty lattice energy.
EL_AB_C: Coulomb component of inter-helical empty lattice energy.
EL_AB_H: HB component of inter-helical empty lattice energy.
Tot_Ss: Total SC-SC interaction energy.  Sc is the same as Scsc in previous scripts, but only 2 letters to save room.  Note that this term, and all subsequent terms related to SC-SC interactions energies, is divided by 2 for the ChainA, ChainB and each residue cases.  This way, the sum of ChainA and ChainB energies equal the total interaction energy for the whole system (since there are only 2 chains).
Ss_AA: Intra-helical sidechain-sidechain interaction energy.   
Ss_AA_V: VDW component of intra-helical sidechain-sidechain interaction energy.
Ss_AA_C: Coulomb component of intra-helical sidechain-sidechain interaction energy.
Ss_AA_H: HB component of intra-helical sidechain-sidechain interaction energy.
Ss_AB: Inter-helical sidechain-sidechain interaction energy.
Sc_AB_V: VDW component of inter-helical sidechain-sidechain interaction energy.
Sc_AB_C: Coulomb component of inter-helical sidechain-sidechain interaction energy.
Sc_AB_H: HB component of inter-helical sidechain-sidechain interaction energy.
Tot_E: Total energy of all components.
...
def main():
    ...
    # Very last: BiHelix output
    output_file = 'Bihelical-E.txt'
    BiHelix_Modules.Print_BiHelix_Final_Residue_Energy_Breakdown(SCREAM_MODEL, scream_EE, output_file)
...
def setup_polar_libraries_dict(Libraries_Dict):
    ...
    for mI in Libraries_Dict.keys():
        mI_string = mI
        if mI_string[0] in ['D', 'E', 'H', 'K', 'N', 'Q', 'R', 'S', 'T', 'Y']:
...
def init_core_polar_EE(SCREAM_MODEL, tmpPtn, Libraries_Dict, mutInfo_rotConnInfo_Dict):
    ...
    for mI in Libraries_Dict.keys():
        ...
        if mI_string[0] in ['D', 'E', 'H', 'J', 'K', 'N', 'Q', 'R', 'S', 'T', 'Y']:
            polar_Libraries_Dict[mI] = Libraries_Dict[mI]
...
def polar_res_pair_priority(ptn, Libraries_Dict, ResidueReachFile, ResidueExclusionFile):
    ...
    AA123_map = {'S':'SER', 'T':'THR', 'C':'CYS', 'D':'ASP', 'E':'GLU', 'H':'HIS', 'J': 'HIS', 'K':'LYS', 'N':'ASN', 'Q':'GLN', 'R':'ARG', 'Y':'TYR'}

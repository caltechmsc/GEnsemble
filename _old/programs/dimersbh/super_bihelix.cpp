                ...
                for (d2 = 1; d2 <= eta_num[2]; d2++)
                {
                  ...
                  { 
                    ...
                    if (i == hel1)
                    {
                      ...
                      thet_new1 = thet[i] + thet1[b1];
                      hpm_new1 = hpm1[c1];
                      eta_new1 = eta[i] + eta1[d1];
                      sprintf (pr,"*  %itmpl    %.2f    %.2f   %.2f    %.2f   %.2f   %.2f    %s  %s",i,xx[i],yy[i],hpm_new1,thet_new1,phi_new1,eta_new1,res[i],resn[i]);      
                      tempout << pr << endl;
                    } 
                    if (i == hel2)
                    {
                      phi_new2 = phi[i] + phi2[a2]; 
                      thet_new2 = thet[i] + thet2[b2];
                      hpm_new2 = hpm2[c2];
                      eta_new2 = eta[i] + eta2[d2];
                      sprintf (pl,"*  %itmpl    %.2f    %.2f   %.2f    %.2f   %.2f   %.2f    %s  %s",i,xx[i],yy[i],hpm_new2,thet_new2,phi_new2,eta_new2,res[i],resn[i]);
                      tempout << pl << endl;
                    }
                  }
                ...
                else 
		{
		  ...
		  system("rm -f CRASH.bgf");
		      ...
		      intr11 = 5000;
                    ...
                    sprintf(oute,"  %.0f    %.0f   %.1f    %.0f   %.0f  %.0f    %.1f    %0.f          %.1f         %.1f        %.1f         %.1f        %.1f        %.1f        %.1f        %.1f        %.1f",phi1[a1],thet1[b1],hpm1[c1],eta1[d1],phi2[a2],thet2[b2],hpm2[c2],eta2[d2],intr11,intr22,inter12,tote,intrv11,intrv22,interv12,novdw,interh12);
                    energyout << oute << endl;

...
Usage: run_super_sbhrank.pl -b {bgf} -m {mfta file} --range {range} -i {input} --ppn
...
for ($i = 1; $i <= $proc; $i++)
{
  ...
  print PBS "/project/Biogroup/Software/GEnsemble/programs/superbihelix/super_sbhrank_dev.pl -b $bgf -m $mfta -n1 $num1 -n2 $num2 -p $i -f $fin -i $totin3\n\n";
